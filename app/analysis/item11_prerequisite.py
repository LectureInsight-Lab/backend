...
절차:
  ...
  3. 하위 25% 백분위수 미만 & 좌우 이웃보다 낮은 지역 극소값 = 주제 전환점
     (연속 저유사도 구간에서 하나의 대표 전환점만 선택)
...
_PREREQ_RE = re.compile(
    # 시간 참조 (아까, 방금 전에, 예전에)
    r"아까.{0,40}(있었|봤|했|봤는데|했는데|얘기|설명|말씀|봤잖|했잖|나왔|나온|리스트|메소드|거죠|하셨|하셨죠|보면서|했고|봤고|그랬잖)"
    r"|아까도\b"
    r"|아까는.{0,30}(잖아요|잖아\b)"
    r"|방금 전에"
    ...
    r"|배웠잖|출력해봤잖|따라해봤잖"
    r"|해봤으니까|만졌으니까|해보셨으니까|배웠으니까|봤으니까|했으니까"
    ...
    # 앞서/앞에/이전에
    r"|앞.{0,3}(서|에|의).{0,15}(봤던|했던|설명|배운|다룬|언급|말씀|봤는데|한번)"
    r"|이전에.{0,15}(배운|했던|봤던|설명|다룬|말씀|봤는데)"
    r"|이미.{0,8}(배운|했던|봤던|아시는|알고|됐|배웠|다뤘)"
    r"|앞서 설명|이전에 말씀"

    # 우리가 + 과거 행위 (동료 학습 참조)
    r"|우리가.{0,15}(했던|봤던|배웠던|다뤘던|해봤던|써봤던|알았어|알았던)"

    # 알려줬던 / 알려드렸던
    r"|알려.{0,5}(줬던|드렸던|줬으니까|드렸으니까|줬잖아|드렸잖아)"

    # 이미 다 알고 있는 전제 표현
    r"|이미.{0,10}(됐(으니|잖|죠)|알고 있(으니|잖|죠)|다뤘(으니|잖|죠))"
)
...
def score_prerequisite(txt_path: str | Path, mode: str = "") -> dict:
    ...
    # Step 3: 전환점 탐지 — 하위 25% 이면서 좌우 이웃보다 낮은 지역 극소값만 선택
    # 연속 저유사도 구간(같은 주제 내 변동)에서 하나의 대표 전환점만 잡아 FP 감소
    threshold = float(np.percentile(sims, SHIFT_PERCENTILE))
    shift_indices = [
        i for i, s in enumerate(sims)
        if s < threshold
        and (i == 0 or sims[i - 1] >= s)
        and (i == len(sims) - 1 or sims[i + 1] >= s)
    ]
