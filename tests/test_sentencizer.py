...
검증:
  ...
  - build_sentences: Kiwi 문장 분리 + Kiwi EF/EC 완결성 판정
      · 'EF' 종결 → is_complete True
      · 'EC' 종결 → is_complete False
  - ends_with_gap: 세그먼트 끝(뒤 갭) 문장에 끊김 플래그
  - completeness_rate: 0~100 범위

Kiwi(kiwipiepy) 미설치 환경에서는 자동 skip.
"""
...
def test_complete_ending_is_ef():
    ...
    sents = sentencizer.build_sentences(utts, use_morph=True)
    assert len(sents) == 1
...
def test_use_morph_false_skips_completeness():
    utts = [_utt("09:00:00", 0, "오늘은 자바를 배웁니다")]
    sents = sentencizer.build_sentences(utts, use_morph=False)
